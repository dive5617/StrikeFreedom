...
CPriorityQueue::CPriorityQueue(const int maxsize) :m_nMaxSize(maxsize)
{
...
{
	const double lfFlankingMassTol = 1.0;  // 0.001
	vector<TAGRES>::iterator it;
	int idx = 0;
	for(it = m_qTags.begin(); it != m_qTags.end(); ++it)
	{
		if( it->strTag.compare(addTag.strTag) == 0 && 
			fabs(it->lfFlankingMass[0] - addTag.lfFlankingMass[0]) < lfFlankingMassTol &&
			fabs(it->lfFlankingMass[1] - addTag.lfFlankingMass[1]) < lfFlankingMassTol &&
...
void CPriorityQueue::Print()
{
	...
	for(it = m_qTags.begin(); it != m_qTags.end(); ++it)
	{
		m_pTrace->info("%s %f %f %f ", it->strTag.c_str(), it->lfScore, it->lfFlankingMass[0], it->lfFlankingMass[1]);
		//for(int i = 0; i < (int)(it->vmodID.size()); ++i)
		//{
		//	m_pTrace->info("%d ", it->vmodID[i]);
		//}
	}
}
...
void CPriorityQueue::SortTagsBySeq()
{
	sort(m_qTags.begin(), m_qTags.end(), TAGRES::TagSeqLess);
}