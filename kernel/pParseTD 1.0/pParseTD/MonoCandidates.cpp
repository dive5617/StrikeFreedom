...
CMainFlow::CMainFlow()
{
	m_vIPV = new double*[IPV_MASSMAX];
	for(int i = 0; i < IPV_MASSMAX; ++i)
	{
		m_vIPV[i] = new double[2 * IPV_PEAKMAX];
	}
...
void CMainFlow::ProcessEachMS1(string filename)
{
	...
	try{
		// initial MS1, output pf1 and pf1idx

		int slashfound = filename.find_last_of("/\\");
		string FileName = filename.substr(slashfound + 1);

		// Get the MS1 file name, with path in it! 
		string FilePath = filename.substr(0, slashfound + 1);
		...
		{
			// .CSV file already exists, pParse Complete!
			cout << "[pParseTD] " << PrecursorslistFilePath_NameCSV << " already exists, pParse Complete!" << endl;
			return;
		}

		MS1Reader MS1(filename, m_para, m_vIPV, m_cEmass, ExtendHalfwindow + atof(m_para->GetValue("isolation_width").c_str()) / 2);

		MS1.SetSpectrumTitle(FileName.substr(0, FileName.size() - 4));
		MS1.SetClassifier(m_para->GetValue("outputpath")); 
		CspecDataIO *outInfo = new CspecDataIO;
	    outInfo->OutputMS1ToPF(MS1,m_para->GetValue("outputpath") + MS1.SpectrumTitle + ".pf1"); // 
		delete outInfo;

		long long length1 = CTools::OutputFilesize(filename.c_str());
		long long length2 = CTools::OutputFilesize(ms2filename.c_str());
		//vector<string> outputList;
		if(length1 > MAX_FILE_SIZE || length2 > MAX_FILE_SIZE){
			cout << "[pParseTD] MS2 file is too large, start to split it..." << endl;
			vector<string> MS2SplitList;
			int filenum = MS2Reader::MS2Split(ms2filename, MS2SplitList);

			FlowKernel(filename, MS1, MS2SplitList[0], true);
			string cmddeletems2 = "del " + MS2SplitList[0];
			system(cmddeletems2.c_str());
			for(int f=1; f<filenum; ++f){
				FlowKernel(filename, MS1, MS2SplitList[f], false);
				cmddeletems2 = "del " + MS2SplitList[f];
				system(cmddeletems2.c_str());  // delete tmp ms2 file
			}
			// merge MGF by activation type e.g. CID, ETD, HCD, if we use output_mgf
			if(m_para->GetValue("output_mgf") == "1"){
				for(int i = 0; i < OutPutFileTypeNum; ++ i){
					bool ExitFlag = false;
					for (int j = 0; j < filenum; j++)
					{
						string CurMGF = tmp +"_ID_" + CTools::int2string(j) + MGFSuffix[i];
						if(!(CTools::IsFileExists(CurMGF, "r")) && j == 0)
						{
							ExitFlag = true;
							break;
						}
					...
					string targetMGF = tmp + MGFSuffix[i];
					FILE * outMGF = fopen(targetMGF.c_str(),"wb");
					for(int j=0; j<filenum; ++j){
						string CurMGF = tmp +"_ID_" + CTools::int2string(j) + MGFSuffix[i];
						FILE *pcur= fopen(CurMGF.c_str(),"rb");
						fseek(pcur, 0L, SEEK_END);
						long fileLen = ftell(pcur);
						fseek(pcur, 0L, SEEK_SET);
						char *buf = new char[fileLen+1];
						fread(buf, sizeof(char), fileLen, pcur);
						fclose(pcur);
						fwrite(buf, sizeof(char), fileLen, outMGF);
						delete []buf;
					}
					fclose(outMGF);
					//string mergecmd = "copy \"" + tmp+"_ID_*"+ MGFSuffix[i]+"\" " + ;
					//plog->printInfoTo(cout, "Info", mergecmd);
					//system(mergecmd.c_str());
			
				}		
			}
			// Merge PF by activation type e.g. CID ETD HCD
			for (int i = 0; i < OutPutFileTypeNum; i ++)
			{
					bool ExitFlag = false;
					int TotalScan = 0;
					for (int j = 0; j < filenum; j++)
					{
						string CurPF = tmp +"_ID_" + CTools::int2string(j) + PFSuffix[i];
						if(!CTools::IsFileExists(CurPF,"rb") && j == 0)
						{
							ExitFlag = true;
							break;
						}
						CPFReader pfr(CurPF);
						TotalScan += pfr.ScanCounts;
					}
					if (ExitFlag)
					{
						continue;
					}
					long SpectraInfoOffSet = 0;

					string TargetPF = tmp +PFSuffix[i];
					string TargetPFIDX = TargetPF + "idx";
					FILE * outPF = fopen(TargetPF.c_str(),"wb");
					FILE * outPFIDX = fopen(TargetPFIDX.c_str(),"wb");

					cout << filenum << " pf2 file will be merged... " << endl;
					for (int j = 0; j < filenum; j++)
					{
						string CurPF = tmp +"_ID_" + CTools::int2string(j) + PFSuffix[i];
						CPFReader pfr(CurPF);
						...
						PFHeadInfo pfh;
						pfr.GetHeadInfo(pfh);
						//cout << pfh.nScanCounts << "," << pfh.nTitleLength << "," << pfh.sTitle << endl;
						if (j == 0){
							pfr.WriteHeadToPF(outPF, pfh, TotalScan);
							SpectraInfoOffSet = pfh.nOffSet;
						}
						pfr.WriteSpectraIDXToPF(outPF, outPFIDX, pfh.nOffSet, SpectraInfoOffSet);
						
					}
					fclose(outPF);                                                    // close files
					fclose(outPFIDX);
					string deletefile = "del ";
					deletefile += tmp +"_ID_*"+PFSuffix[i];
				
					deletefile = deletefile.substr(0,deletefile.size()-3) +"*";   // delete _ID_.*, including mgf and pf2
					system(deletefile.c_str());
					CPFReader pftest(TargetPF);
					if(pftest.ValidPF())
						cout << TargetPF << " is a Valid PF file." << endl;
			}		

		}else{
			FlowKernel(filename, MS1, ms2filename, true);
		}
	}catch(exception &ex){
		CErrInfo info("CMainFlow", "ProcessEachMS1", "");
...
void CMainFlow::GetStart()
{
	try
	{
		string IPVfilepath = m_para->GetValue("ipv_file");
...
*/
void CMainFlow::FlowKernel(string MS1file, MS1Reader &MS1, string MS2file, bool newModel)
{
	...
	for (int file_idx = 0; file_idx < OutPutFileTypeNum; ++file_idx)   /* For each activation type! */
	{
		...
		for (MS2Index = 0; MS2Index < MS2Scancounts ; ++MS2Index)        /* For each MS/MS spectrum of current activationtype! */
		{
			...
			{                                                        /* Speed | Schedule | Rate of Process */  
				...
				printf("%.0f%%\r", (MS2Index + 1) * 100.0 / MS2Scancounts);
				fflush(stdout);
			}
		}
		...
		//fprintf(stdout, "\r%s-%s MS1：%.0f%%", cActivateType[(file_idx)%ActivateTypeNum], cInstrumentType[(file_idx)/ActivateTypeNum], ProcessedMS2*100.0/MS2Scancounts);
		printf("[pParseTD] <Exporting %s-%s MS1>: ", cActivateType[file_idx % ActivateTypeNum], cInstrumentType[file_idx / ActivateTypeNum]);
		printf("%.0f%%\r", MS2Scancounts * 100.0 / MS2Scancounts);
		fflush(stdout);
		cout << endl;
		...
		MS1.MacLearnClassify(m_para->GetValue("model_type"),newModel);
		...
		for (int k = 0; k < (int)MS1.m_vnIndexListToOutput.size(); ++k)
		{
			...
			{   /* Print the Rate of Process */  
				...
				printf("%.0f%%\r", (k+1) * 100.0 / (int)MS1.m_vnIndexListToOutput.size());
				fflush(stdout);
			}
		}
        // #test
	...
	{
		ofstream csvout;
		csvout.open(PrecursorslistFilePath_NameCSV.c_str(), ios::out|ios::app);
