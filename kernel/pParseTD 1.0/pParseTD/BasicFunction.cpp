...
{
	tm exptm;
	// make the eternal version 
	exptm.tm_year = 2018 - 1900;   /* years since 1900 */
	exptm.tm_mon = 11;     /* months since January - [0,11] */
	exptm.tm_mday = 31;
	exptm.tm_hour = 0;
	exptm.tm_min = 0;
	exptm.tm_sec = 0;
...
void CHelpInfo::WhatsNew()
{
	...
	cout << "\t[2014.11.26] pParseTD 1.0.1 was available" << endl;
...
void CParaProcess::CheckParam()
{
	...
	if (_isPath(DataPath))
	{
		...
		if(fileExt.compare(".raw") == 0 || fileExt.compare(".ms1") == 0)
		{
			...
			cout << "[Error] Invalid datapath 2: " << DataPath << endl;
...
{
	map<string, string>::iterator it = m_mapPara.find(strKey);
	if(it != m_mapPara.end())
	{
		return it->second;
	} else {
