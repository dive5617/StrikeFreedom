...
void CEmass::Init()
{
	...
	double Model[m_nElemNum] = {4.9384, 7.8602, 1.2758, 1.4202, 0.03168};  // 平均氨基酸分子式的下标？
	...
	tmp.push_back(CentroidPeaks(34.967,0));  // added by wrm @2016.07.25
	...
	vector<int> pattern;
	GetFormbyAvegine((double)m_nBase, pattern);
	// 计算平均氨基酸分子的同位素峰簇
	for(size_t i = 0; i < pattern.size(); ++i)  // C/H/N/O/S
	{
		//cout << pattern[i] << endl;
...
void CEmass::Convolution(const vector<CentroidPeaks> &comA, const vector<CentroidPeaks> &comB, vector<CentroidPeaks> &res)
{
	...
	for(int i = 0; i < (int)comA.size(); ++i)
	{
		...
		for(int j = 0; j < (int)comB.size(); ++j)
		{
			...
			ym = comB[j].m_lfMZ;
			if(ym < 0)   continue;
			z = x * y; // 如何防止z超过double表示的最小范围？  [wrm]: 强度乘积后岂不是更小了？
...
void CEmass::Calculate(double mass, vector<CentroidPeaks> &res)
{
	...
	vector<CentroidPeaks> oneElemRes;
	for (int i = 0; i+1 < 2 * IPV_PEAKMAX; i += 2)
	{
		CentroidPeaks tmp(m_vIPV[calmass][i], m_vIPV[calmass][i+1]);
		oneElemRes.push_back(tmp);
	}
