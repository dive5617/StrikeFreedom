...
MS2Spectrum::MS2Spectrum(int CurrentMS2Scan, vector<unordered_map<int,vector<CentroidPeaks>>>& vIPV, CEmass *cemass)
...
void MS2Spectrum::GetEachIsoPeak(int peakPos, int chg, int &isoPeakNums, double &stdPPMTol, double &sumPeakInens, vector<double> &isoPattern, vector<int> &tmpIsoform, double Peak_Tol_ppm)
{
	...
	for(int s = HalfIsoWidth - 1; s >= 0; --s) // 从最高峰处向前查找
	{
		...
		if(missNum >= 2)  // 如果后面有两根谱峰缺失，则舍弃前面所有谱峰
		{
			isoPattern[s] = 0.0;
			tmpIsoform[s] = -1;
		} else {
	...
	for(int s = HalfIsoWidth + 1; s < (int)isoPattern.size(); ++s) // 从最高峰处向后查找
	{
		...
		if(missNum >= 2)  // 如果前面有两根谱峰缺失，则舍弃后面所有谱峰
		{
			isoPattern[s] = 0.0;
			tmpIsoform[s] = -1;
		} else {
	...
	{   // 超出IPV的长度
		...
		for(it = m_vIPV[mIdx].begin(); it != m_vIPV[mIdx].end(); ++it){
			...
			for(int i=0; i<IPV_PEAKMAX; ++i){
				thoPattern.push_back(it->second[i].m_lfIntens);
			}
			int limit = 1;
			if(highestPeakPos == 0) limit = 0; // 最高峰为mono峰，则不往左右移动
			else if(mass > 3383) limit = 2;
			// gap校准
			for(int m = -limit; m <= limit; ++m)
			{
				if(isoPattern[HalfIsoWidth + m] < eps) // 左右没有峰
				{
					continue;
				}
				...
				{    
					if(vTheo[k] / vTheo[HalfIsoWidth + m] < 0.01 && isoPattern[k] > eps)  // 0.05
					{
				...
				// oneisoPeakNum > 1  modified by wrm @2016.05.12
				if(oneisoPeakNum > 1 && similarity > bestSim)
				{
					bestSim = similarity;
...
double MS2Spectrum::FilterByBaselineWhole(double SNratio)
{
	...
	if(removePeakID.size() == 0) return pow(10, baseline);
	size_t pos = 0, removePos = 0;
	for(size_t i = 0; i < Peaks.size(); ++i)
	{
		if(removePos < removePeakID.size() && i == removePeakID[removePos])
...
void MS2Spectrum::Deconvolution(int precurChg, vector<double> &precurMass, double Peak_Tol_ppm, double SNratio)
{
	...
	vector<int> vIntensOrder;
	vIntensOrder.assign(peakNum, 0);
	...
	// sort all peaks by intensity, and the order recode in the array?
	...
	for(int k = 0; k < peakNum; ++k) // 枚举每根峰为最高峰
	{
		...
		{
		    int isoPeakNums = 0;
			double stdPPMTol = 0.0;  // 平均同位素模式偏差
			...
			if(monoMass * chg - (chg - 1) * pmass > 6000 && isoPeakNums < 5) //TO CHECK
				continue;
			if(chg == 1 && monoMass * chg - (chg - 1) * pmass > 1500 && isoPeakNums == 1) // [wrm?]:单电荷的单一谱峰若质量大于1500暂不考虑？
				continue;
