...
MS1Reader::MS1Reader(string &infilename, CParaProcess *para, vector<unordered_map<int,vector<CentroidPeaks>>>& vIPV, CEmass *cEmass, double halfWnd) : 
	m_lfHalfWnd(halfWnd), m_PeakMap(MaxBinPerTh, Span, halfWnd), m_cEmass(cEmass), m_para(para), m_vIPV(vIPV)
{
	...
	IsoPattern.assign(MaxIsoFeature, 0);    // 同位素峰簇模式 15*11 = 165
...
void MS1Reader::GetCandidatesPrecursorByMS2scan(int MS2scan, int MS1scan, int userSpanStart,int userSpanEnd,double ActivationCenter,vector<CentroidPeaks> &PrecursorCandidatePeaks)
{
	...
	try{
		...
		for ( int j = userSpanStart + 1 ; j <= userSpanEnd && MS1_idx + j < MS1Scancounts ; ++j )
		{
			...
			if (j >= -1 && j <= 1)
			{
				PeakEndIndex.push_back(PrecursorCandidatePeaks.size());//记录每一张谱图中谱峰在PrecursorCandidatePeaks中的结束位置。便于删除。
			}
		}
	...
	{
		CErrInfo info("MS1Reader", "GetCandidatesPrecursorPeaks", "Get Candidate Precursor Peaks failed.");
		throw runtime_error(info.Get(e).c_str());
		//throw ("CMainFlow::FlowKernel() output .mgf/.pf files failed.");
	}
	catch(...)
	{
		CErrInfo info("MS1Reader", "GetCandidatesPrecursorPeaks", "Caught an unknown exception from get candidate precursor peaks.");
...
void  MS1Reader::GetAllIsoFeature(int chg, double ActivationCenter, vector<CentroidPeaks> &PrecursorCandidatePeaks, int k)
{
	...
	{   // 循环查看目前一级谱的前span和后span张一级谱
		...
		{   // 对枚举的最高峰位置，寻找其前HalfIsoWidth(0.5 + isolation/2)和后HalfIsoWidth根谱峰
			...
			{
				break;
			} else if(jmpIndex < 1) {   // [wrm?] jmpIndex可以取0？
				jmpIndex = 1;
			}
			...
			{
				IsoPattern[basePos + s + HalfIsoWidth] += m_PeakMap.GetPeak(inoSpan, j); // [wrm?] 取偏差最小的谱峰			
			} // end for j		
		} // end for s
	} // end for inoSpan
}

// 计算最高峰及左右几根峰的色谱相似度(余弦相似度) modified by luolan @2014.05.12
void MS1Reader::CalcChromSimilarity()
{
	...
	if(fabs(highestSim) < eps) // highestSim == 0.0 浮点数最好不要直接判断是否相等
	{
		return;
	}
	for(int i = 0; i < (int)vTmpXYSimilarity.size(); ++i) 
	{
		if(fabs(vTmpXXSimilarity[i]) < eps || i == PeaksforChromSim)
		{
			continue;
		}
							   ...
							   double &stdPPMTol, double &sumMatchedInt, double prePeakMz, bool &isIncluded)
{
	...
	for (int s = -HalfIsoWidth; s <= HalfIsoWidth; ++s)
	{
		...
		//	}
		//}
		int idx = BinarySearchPeak(PrecursorCandidatePeaks, targetMZ, Peak_Tol_ppm);
		if(idx >= 0){
			minBias = fabs(PrecursorCandidatePeaks[idx].m_lfMZ - targetMZ);
			IsoPattern[s + HalfIsoWidth] = PrecursorCandidatePeaks[idx].m_lfIntens;
			for(int i = idx - 1; i >= 0; --i){
				double tmpBias = fabs(PrecursorCandidatePeaks[i].m_lfMZ - targetMZ);
				if(tmpBias > TolValueTh){
					break;
				}else if(tmpBias < TolValueTh){
					minBias = tmpBias;
					IsoPattern[s + HalfIsoWidth] = PrecursorCandidatePeaks[i].m_lfIntens;
				}
			}
			for(int i = idx + 1; i < PrecursorCandidatePeaks.size(); ++i){
				double tmpBias = fabs(PrecursorCandidatePeaks[i].m_lfMZ - targetMZ);
				if(tmpBias > TolValueTh){
					break;
				}else if(tmpBias < TolValueTh){
			...
			minBias = minBias / targetMZ * 1000000;   // ppm
...
void MS1Reader::GetTheoryIsoform(double mass, int &highestPeakPos, vector<double> &vTheoIso)
{
	...
	int mIdx = int(mass + round_off);
	if(mIdx >= LIPV_MASSMAX || mIdx < 0)
	{
		return;
	} else if (mIdx < LIPV_MASSMAX) {   // 小于100K则直接读取
		//cout << mIdx << endl;
		unordered_map<int,vector<CentroidPeaks>>::iterator it;
		for(it = m_vIPV[mIdx].begin(); it != m_vIPV[mIdx].end(); ++it){
			highestPeakPos = it->first;
			for (int i = 0; i < IPV_PEAKMAX; ++ i)
			{
				vTheoIso.push_back(it->second[i].m_lfIntens);
			}
			break;
		}
	} else {  // 大于等于100K则在线生成
...
//从给定文件F:\Skydrive\coding\pParsesrc\IPV.txt中获取平均氨基酸模型的结果。并存放到全局变量IPV[10000][38]中。其中行代表质量的整数值，列代表同位素模式。没有归一化。
double MS1Reader::ComputIPVSimilarity(double &SumCurPrecursorInt, vector<double> &vTheoIso, double &IsoLenScore)
{
	...
	for (int j = 0; j < 2*Span+1; ++j) // 前后10张一级谱，加上候选谱峰列表中的准同位素峰簇
	{
		V2.clear();
		for(int k = 0; k < IsoProfileNum; ++k)
		{
...
void MS1Reader::CutSamePrecursor(vector<CentroidPeaks> &PrecursorCandidatePeaks)
{
	...
	double tol=PrecursorCandidatePeaks[0].m_lfMZ*Peak_Tol_ppm;  // 谱峰误差
    ...
    // modified by wrm. @2016.03.16
	double sumIntensity = PrecursorCandidatePeaks[0].m_lfIntens;
	double sumMz = PrecursorCandidatePeaks[0].m_lfMZ*PrecursorCandidatePeaks[0].m_lfIntens;
	for (unsigned int i = 1; i < PrecursorCandidatePeaks.size(); ++i)
	{
		if(fabs(PrecursorCandidatePeaks[LastIndex].m_lfMZ - PrecursorCandidatePeaks[i].m_lfMZ) < tol)
		{
				DeleteList.push_back(i);
				sumMz +=(PrecursorCandidatePeaks[i].m_lfIntens*PrecursorCandidatePeaks[i].m_lfMZ);				
				sumIntensity += PrecursorCandidatePeaks[i].m_lfIntens;	
				PrecursorCandidatePeaks[LastIndex].m_lfIntens = max(PrecursorCandidatePeaks[LastIndex].m_lfIntens, PrecursorCandidatePeaks[i].m_lfIntens);
		} else {
			PrecursorCandidatePeaks[LastIndex].m_lfMZ = sumMz / sumIntensity;
			LastIndex=i;
			sumIntensity = PrecursorCandidatePeaks[i].m_lfIntens; 
			sumMz = PrecursorCandidatePeaks[i].m_lfMZ*PrecursorCandidatePeaks[i].m_lfIntens;
		}
	}
...
// 在给定谱峰列表中二分查找指定的谱峰
int MS1Reader::BinarySearchPeak(vector<CentroidPeaks> &allPeaks, double targetMz, double peakTol){
	int left = 0,right = allPeaks.size()-1;
	double tolValueTh = targetMz * peakTol;
	while(left < right){
		int mid = (left + right) >> 1;
		if((allPeaks[mid].m_lfMZ - targetMz) > tolValueTh){
			right = mid - 1;
		}else if((targetMz - allPeaks[mid].m_lfMZ) > tolValueTh){
			left = mid + 1;
		}else{
			return mid;
		}
	}
	if(fabs(allPeaks[left].m_lfMZ - targetMz) <= tolValueTh)   return left;
	return -1;
}

int MS1Reader::SearchPeak(vector<CentroidPeaks> &allPeaks, vector<int> &peakHash, double targetMz, double peakTol)
{
	double tolValueTh = targetMz * peakTol;
	int tar = int(targetMz*FACTOR);
	int lower = (int)((targetMz - tolValueTh)*FACTOR), upper = (int)((targetMz + tolValueTh)*FACTOR);
	int pos = peakHash[tar];
	if (pos > -1)  return pos;
	int dis = INT_MAX;
	for (int i = lower; i < upper; ++i){
		if (peakHash[i] > -1 && abs(tar-i) < dis){
			dis = abs(tar - i);
			pos = peakHash[i];
		}
	}
...
bool compHighIntens(const PrecursorPeakIntensity &x, const PrecursorPeakIntensity &y)
{
	return x.m_lfIntens > y.m_lfIntens;
}

//void MS1Reader::ReconstructIsotopePeakCluster(vector<CentroidPeaks> &PrecursorCandidatePeaks, vector<CandidatePrecursorPeak> &candidatePrecursorPeaks)
//{
//	int n = PrecursorCandidatePeaks.size();
//	vector<bool> is_used(n,false);
//	//int chargeStart = 2;
//	const int chgEnd = atoi(m_para->GetValue("max_charge", "30").c_str());
//	//vector<CentroidPeaks> iso_pattern;
//	const int topK = 3;
//	vector<PrecursorPeakIntensity> tp;
//	vector<int> peakIdxs;
//	//假设每种电荷状态下，每根峰只属于一个同位素峰簇
//	for(int chg = chgStart; chg <= chgEnd; ++ chg){  // 枚举电荷
//		is_used.assign(n,false);
//		for(int i=0; i<n; ++i){   // 枚举谱峰
//			if(is_used[i])   continue;
//			tp.clear();
//			int peaknum = 1;
//			tp.push_back(PrecursorPeakIntensity(i, PrecursorCandidatePeaks[i].m_lfIntens));
//			is_used[i] = true;
//			peakIdxs.clear();
//			for(int s = 1; ; ++ s){
//				double targetMz = PrecursorCandidatePeaks[i].m_lfMZ + avgdiv * s / chg;
//				if(targetMz > PrecursorCandidatePeaks.back().m_lfMZ)   break;
//
//				int idx = BinarySearchPeak(PrecursorCandidatePeaks, targetMz);
//				if(idx >= 0){
//					++ peaknum;
//					peakIdxs.push_back(idx);
//					tp.push_back(PrecursorPeakIntensity(idx, PrecursorCandidatePeaks[idx].m_lfIntens));
//				}
//			}
//			// TODO: 暂定5吧
//			if(peaknum >= 3){
//				sort(tp.begin(), tp.end(), compHighIntens);
//				for(int hp = 0; hp < topK; ++ hp){
//					candidatePrecursorPeaks.push_back(CandidatePrecursorPeak(tp[hp].peakIdx, chg));
//				}
//				for(int hp = 0; hp < peakIdxs.size(); ++ hp){
//					is_used[peakIdxs[hp]] = true;
//				}
//			}
//		}	
//	}
...
void MS1Reader::DetectIsotopePeakCluster(vector<CentroidPeaks> &PrecursorCandidatePeaks, vector<CandidatePrecursorPeak> &candidatePrecursorPeaks, const double baseline)
{
	int n = PrecursorCandidatePeaks.size();
	// get local extremum value
	vector<int> localPeaks;
	// 1.003/60 = 0.0167, 100*20.0/1000000 = 0.002, 2000*20.0/1000000 = 0.04, 当电荷数目较多时，20ppm的误差范围会使得误把同位素峰当作当前峰，即区分不开
	vector<int> peakHash(MaxMZ*FACTOR, -1);
	peakHash[(int)PrecursorCandidatePeaks[0].m_lfMZ * FACTOR] = 0;
	if(n > 1 && PrecursorCandidatePeaks[0].m_lfIntens > PrecursorCandidatePeaks[1].m_lfIntens && PrecursorCandidatePeaks[0].m_lfIntens > baseline){
		localPeaks.push_back(0);
	}
	for(int i=1; i<n-1; ++i){
		peakHash[PrecursorCandidatePeaks[i].m_lfMZ * FACTOR] = i;
		if(PrecursorCandidatePeaks[i].m_lfIntens > baseline && PrecursorCandidatePeaks[i].m_lfIntens > PrecursorCandidatePeaks[i-1].m_lfIntens && PrecursorCandidatePeaks[i].m_lfIntens > PrecursorCandidatePeaks[i+1].m_lfIntens){
			localPeaks.push_back(i);
		}
	}
	peakHash[(int)PrecursorCandidatePeaks[n - 1].m_lfMZ * FACTOR] = n - 1;
	if(n > 1 && PrecursorCandidatePeaks[n-1].m_lfIntens > PrecursorCandidatePeaks[n-2].m_lfIntens && PrecursorCandidatePeaks[n-1].m_lfIntens > baseline){
		localPeaks.push_back(n-1);
	}
	...
	vector<PrecursorPeakIntensity> tp;
	//假设每种电荷状态下，每根峰只属于一个同位素峰簇
	double peakTol = Peak_Tol_ppm;	
	for(int chg = chgStart; chg <= chgEnd; ++ chg){  // 枚举电荷
		is_used.assign(n,false);
		for(int k=0; k<localPeaks.size(); ++k){	
			int i = localPeaks[k];
			if(is_used[i])   continue;
			int mz = min((int)PrecursorCandidatePeaks[i].m_lfMZ, mz_end);
			if(chg > 7) peakTol = min(Peak_Tol_ppm, Tables::s_vf_charge_mz_tol[mz-mz_beg][chg-chg_beg]);
			int contiPeaks = 0, peaknum = 0, hole = 0;
			int s;
			is_used[i] = true;
			peakIdxs.clear();
			tp.clear();
			...
			for(s = -7; s < 9; ++ s){
				...
				double targetMz = PrecursorCandidatePeaks[i].m_lfMZ + avgdiv * s / chg;
				if (targetMz < PrecursorCandidatePeaks[0].m_lfMZ){
					if (abs(s) <= 3) ++hole;
					continue;
				}
				if (targetMz > PrecursorCandidatePeaks.back().m_lfMZ){
					if (abs(s) <= 3) ++hole;
					break;
				}
				int idx = SearchPeak(PrecursorCandidatePeaks, peakHash, targetMz, peakTol); //BinarySearchPeak(PrecursorCandidatePeaks, targetMz, peakTol);
				if(idx >= 0 && abs(s) > 3 && PrecursorCandidatePeaks[idx].m_lfIntens > PrecursorCandidatePeaks[i].m_lfIntens*2.0){
					flag = false;
					break;
				}
				if(idx >= 0 && !is_used[idx]){
					++ peaknum;
					peakIdxs.push_back(idx);
					if(PrecursorCandidatePeaks[idx].m_lfIntens > PrecursorCandidatePeaks[i].m_lfIntens){
						tp.push_back(PrecursorPeakIntensity(idx, PrecursorCandidatePeaks[idx].m_lfIntens));
					}
				}else{  // 此处谱峰不连续
					contiPeaks = max(contiPeaks, peaknum);
					peaknum = 0;
					// peakIdxs.push_back(-1);
					if (abs(s) <= 3) ++hole;
				}
			}
			contiPeaks = max(contiPeaks, peaknum);
			if(s < 2 || !flag || contiPeaks < 3)  continue;
			// TODO: 暂定5吧
			if(hole < 5){
				sort(tp.begin(), tp.end(), compHighIntens);
				for(int hp = 0; hp < 4 && hp < tp.size(); ++ hp){
					candidatePrecursorPeaks.push_back(CandidatePrecursorPeak(tp[hp].peakIdx, chg));
				}
				for(int hp = 0; hp < peakIdxs.size(); ++ hp){
					is_used[peakIdxs[hp]] = true;
...
void MS1Reader::ReconstructIsotopePeakCluster(vector<CentroidPeaks> &PrecursorCandidatePeaks, vector<CandidatePrecursorPeak> &candidatePrecursorPeaks, const double baseline)
{
	int n = PrecursorCandidatePeaks.size();
	vector<int> peakHash(MaxMZ*FACTOR, -1);
	for (int i = 0; i<n; ++i){
	...
	//int chargeStart = 2;
	const int chgEnd = atoi(m_para->GetValue("max_charge", "30").c_str());
	unordered_set<CandidatePrecursorPeak> tmpst;
	const int topK = 3;
	int t1 = -1,t2 = -1,t3 = -1;
	vector<int> peakIdxs;
	//假设每种电荷状态下，每根峰只属于一个同位素峰簇
	...
	for(int chg = chgStart; chg <= chgEnd; ++ chg){  // 枚举电荷
		...
		for(int i=0; i<n; ++i){   // 枚举谱峰
			...
			if(chg > 7) peakTol = min(Peak_Tol_ppm, Tables::s_vf_charge_mz_tol[mz-mz_beg][chg-chg_beg]);
			t1= i, t2 = -1, t3 = -1;
			int contiPeaks = 1, peaknum = 1, hole = 0;
			is_used[i] = true;
			peakIdxs.clear();
			for(int s = 1; ; ++ s){
				double targetMz = PrecursorCandidatePeaks[i].m_lfMZ + avgdiv * s / chg;
				...
				if(idx >= 0){   // && !is_used[idx]
					...
					++ peaknum;
					peakIdxs.push_back(idx);
					if(PrecursorCandidatePeaks[idx].m_lfIntens > PrecursorCandidatePeaks[t1].m_lfIntens){
						t3 = t2, t2 = t1, t1 = idx;
					}else if(t2 == -1 || PrecursorCandidatePeaks[idx].m_lfIntens > PrecursorCandidatePeaks[t2].m_lfIntens){
						t3 = t2, t2 = idx;
					}else if(t3 == -1 || PrecursorCandidatePeaks[idx].m_lfIntens > PrecursorCandidatePeaks[t3].m_lfIntens){
						t3 = idx;
					}
				}else{  // 此处谱峰不连续
					contiPeaks = max(contiPeaks, peaknum);
				...
				if(s > 18 || peakIdxs.size() >= 15)  break;
			}
			contiPeaks = max(contiPeaks, peaknum);
			if(PrecursorCandidatePeaks[t1].m_lfIntens < baseline || hole > 5 || contiPeaks < 3)  
				continue;
			// TODO: 暂定5吧
			if(peakIdxs.size() >= 3){
				tmpst.insert(CandidatePrecursorPeak(t1, chg));
				tmpst.insert(CandidatePrecursorPeak(t2, chg));
				tmpst.insert(CandidatePrecursorPeak(t3, chg));
				for(int hp = 0; hp < peakIdxs.size(); ++ hp){
					is_used[peakIdxs[hp]] = true;
				}
			}
	...
	unordered_set<CandidatePrecursorPeak>::iterator it;
	for(it = tmpst.begin(); it != tmpst.end(); ++ it){
		candidatePrecursorPeaks.push_back(*it);
	}
}
...
// modified by wrm. @2016.03.16
void MS1Reader::AssessPrecursors(double ActivationCenter,int MS2scan,int MS1scan,vector<CentroidPeaks> &PrecursorCandidatePeaks,TrainingSetreader &TrainSet)
{
	...
	// 对(前后10张一级谱)碎裂窗口（扩展0.5Da）内的所有谱峰按照mz进行排序，便于后续计算最高峰精度
	...
	vector<CandidatePrecursorPeak> candidatePrecursorPeaks;
	
	double baseline = GetBaselineByIndex(MS1Scan_Index[MS1scan]);
	// 重构同位素峰簇
	DetectIsotopePeakCluster(PrecursorCandidatePeaks, candidatePrecursorPeaks, baseline);
	//ReconstructIsotopePeakCluster(PrecursorCandidatePeaks, candidatePrecursorPeaks, baseline);

	// 计算母离子特征信息
	for(int k = 0; k < candidatePrecursorPeaks.size(); ++ k){
		int idx = candidatePrecursorPeaks[k].peakIdx;  // 最高峰索引
		int chg = candidatePrecursorPeaks[k].charge;   // 电荷
		...
		double stdPPMTol = 0.0;   // 同位素模式偏差
		double SumCurrentPrecursor = 0.0;	// 从获选谱峰列表中获取的同位素峰强度和
		isIncluded = false;

		// 根据最高峰和电荷获取同位素峰簇（共保存15根峰）
		// 并计算在候选谱峰列表中的同位素峰数目、同位素模式偏差、匹配到的谱峰强度和
		GetEachIsoPeak(PrecursorCandidatePeaks, chg, idx, IsoPeakInWindow, stdPPMTol, SumCurrentPrecursor, prePeakMz, isIncluded);
		...
		GetAllIsoFeature(chg, ActivationCenter, PrecursorCandidatePeaks, idx);
		...
		//cout << PrecursorCandidatePeaks[idx].m_lfMZ << endl;
		// 计算理论同位素分布
		GetTheoryIsoform(PrecursorCandidatePeaks[idx].m_lfMZ*chg - chg*pmass, highestPeakPos, vTheoIso);
		if(0 == vTheoIso.size()) continue;
		// 根据理论同位素分布、最高峰位置计算Mono质量
		double monoMass = PrecursorCandidatePeaks[idx].m_lfMZ - highestPeakPos * avgdiv / chg;
		...
		//if(expRelativeInts > 1.0) expRelativeInts = 1.0;  // commented by wrm. @2016.03.17
		...
		if((chg > 3 && IsoLenScore < 0.5) || MaxSim < 0.3)  // 如果匹配到的谱峰数目小于一半，则舍去该母离子
		{
			continue;
		}
		...
		// [wrm] 没有用到  commented by wrm. @2016.03.17
		/*if (pTrainSet != NULL)
		{
		...
		for (int Peak_i = 0; Peak_i < (int)PrecursorCandidatePeaks.size() ; Peak_i++)
		{
				...
				&& PrecursorCandidatePeaks[Peak_i].MZ < ActivationCenter + IsolationWidth/2 )
			{
				...
				{
					if (fabs(PrecursorCandidatePeaks[idx].MZ + IsoPattern_i*avgdiv/chg - PrecursorCandidatePeaks[Peak_i].MZ) < ActivationCenter * Peak_Tol_ppm)
					{
		...
		if (PrecursorCandidatePeaks[idx].m_lfMZ > ActivationCenter + IsolationWidth / 2 + IsolationExtend)
		{
			continue;
		}*/

		/*if (IsoPeakInWindow < 2 && (chg != 2 || PrecursorCandidatePeaks[idx].m_lfMZ > ActivationCenter+IsolationWidth/4))
		{
			continue;
		}*/
		...
		if(MS2scan == 2732)
		{
			cout<<highestPeakPos<<" "<<PrecursorCandidatePeaks[idx].m_lfMZ<<" "<<chg<<" "<<IsoPeakInWindow<<" "<<PIF<<" "<<MaxSim<<" "<<chromMaxSim<<" "<<tmpScore<<endl;
		}
#endif
		if(tmpScore < eps)// bestScore == 0.0
		{
			continue;
		}
		if(isIncluded && tmpScore <= preScore)
		{
			continue;
		} else if(isIncluded){  // 若该谱峰已使用过，则保留打分最高的那个
			prePeakMz = PrecursorCandidatePeaks[idx].m_lfMZ;
			preScore = tmpScore;
			...
			double PeakSTD = CalcSTDforMZ( PrecursorCandidatePeaks[idx].m_lfMZ );  // 最高峰精度, STD值越小越好
			double AccMZ = ReCalcMZ( PrecursorCandidatePeaks[idx].m_lfMZ );  // 最高峰质量校准
			double MonoMz = AccMZ - highestPeakPos * avgdiv / chg;  // * Mono峰的估计质量
			// 所有的特征都归一化
			MarsFeature[last][0] = (chg * 1.0) / chgEnd;
			MarsFeature[last][1] = MaxSim;
			MarsFeature[last][2] = (20.0 - PeakSTD) / 20.0; // changed by wrm
			MarsFeature[last][3] = chromMaxSim;
			MarsFeature[last][4] = (20.0 - stdPPMTol) / 20.0; // CHECK
			MarsFeature[last][5] = closeChgSim;
			MarsFeature[last][6] = (IsoPeakInWindow * 1.0) / IsoProfileNum;
			MarsFeature[last][7] = 1 - tmpMonoDiv; // test 2014.10.08  * 单同位素峰相对强度差
			MarsFeature[last][8] = MonoMz / 2000.0; // CHECK
			MarsFeature[last][9] = IsoLenScore;
			MarsFeature[last][10] = PIF;
			MarsFeature[last][11] = tmpScore;
			...
			OutputInfo[last][1] = (double)chg;
			OutputInfo[last][2] = (double)MonoMz;
			
			last = (int)PrecursorsCandidatesArray.size() - 1;
			PrecursorsCandidatesArray[last][0] = MonoMz;//0
			PrecursorsCandidatesArray[last][1] = chg;//1
			PrecursorsCandidatesArray[last][2] = MaxSim;//2
			PrecursorsCandidatesArray[last][3] = 20.0 - PeakSTD; //(IsoPeakInWindow * 1.0) / IsoProfileNum;//3   modified by wrm.
			PrecursorsCandidatesArray[last][4] = chromMaxSim;//4
			PrecursorsCandidatesArray[last][5] = 20.0 - stdPPMTol;  //5  //将实验同位素峰簇的偏差作为一维特征，主要用于区分相邻的高电荷，打分越高越好 -luolan
			PrecursorsCandidatesArray[last][6] = closeChgSim;//6
			PrecursorsCandidatesArray[last][7] = PIF;//7
		} else {  // 
			prePeakMz = PrecursorCandidatePeaks[idx].m_lfMZ;
			preScore = tmpScore;
			double PeakSTD = CalcSTDforMZ( PrecursorCandidatePeaks[idx].m_lfMZ );  // 最高峰精度
			double AccMZ = ReCalcMZ( PrecursorCandidatePeaks[idx].m_lfMZ );  // 最高峰质量校准
			double MonoMz = AccMZ - highestPeakPos * avgdiv / chg;  // * Mono峰的估计质量
			vector<double> MarsFeatureTmp;
			vector<double> OutputInfoTmp;
			MarsFeatureTmp.push_back((chg * 1.0) / chgEnd);  // 电荷
			MarsFeatureTmp.push_back(MaxSim);         // 同位素模式相似度
			MarsFeatureTmp.push_back((20.0 - PeakSTD) / 20.0);  // 最高峰精度
			MarsFeatureTmp.push_back(chromMaxSim);    // 色谱相似度
			MarsFeatureTmp.push_back((20.0 - stdPPMTol) / 20.0);  // 同位素模式偏差
			MarsFeatureTmp.push_back(closeChgSim);         // 相邻电荷状态的同位素峰簇相似度
			MarsFeatureTmp.push_back((IsoPeakInWindow * 1.0) / IsoProfileNum); // 同位素峰数目
			MarsFeatureTmp.push_back(1 - tmpMonoDiv);    // 单同位素峰相对强度差
			MarsFeatureTmp.push_back(MonoMz / 2000.0);  // 母离子质量
			MarsFeatureTmp.push_back(IsoLenScore);  // 同位素模式长度打分
			MarsFeatureTmp.push_back(PIF);     // 碎裂窗口内谱峰强度比
			MarsFeatureTmp.push_back(tmpScore);   // 粗打分值
			...
			OutputInfoTmp.push_back((double)chg);
			OutputInfoTmp.push_back((double)MonoMz);
			OutputInfo.push_back(OutputInfoTmp);
			
			// Record a precursor
			PrecursorsCounter++;
			PrecursorsCandidatesItem.push_back(MonoMz);//0
			PrecursorsCandidatesItem.push_back(chg);//1
			PrecursorsCandidatesItem.push_back(MaxSim);//2
			PrecursorsCandidatesItem.push_back(20.0 - PeakSTD);//3
			//PrecursorsCandidatesItem.push_back(EluteSim);//(ChromSimilarity[1]);
			PrecursorsCandidatesItem.push_back(chromMaxSim);//4
			//PrecursorsCandidatesItem.push_back(newsumIsopattern);//5
			PrecursorsCandidatesItem.push_back(20.0 - stdPPMTol);//5 
			//将实验同位素峰簇的偏差作为一维特征，主要用于区分相邻的高电荷，打分越高越好 -luolan
			PrecursorsCandidatesItem.push_back(closeChgSim);//6【引入虚拟谱峰】
			PrecursorsCandidatesItem.push_back(PIF);//7
			...
			//bCorrectSample=TrainSet.CheckScanMZCharge(MS2scan,PrecursorCandidatePeaks[idx].MZ,chg);
	...
	for(int k = 0; k < OutputTopN; ++k)
	{
		...
		cout<<k<<": MZ=" <<PrecursorsCandidatesArray[idx][0]
			<<" Charge=" <<int(PrecursorsCandidatesArray[idx][1])
			<< " Mass=" <<(PrecursorsCandidatesArray[idx][0] - pmass)*PrecursorsCandidatesArray[idx][1]
			<<  " maxSim=" << PrecursorsCandidatesArray[idx][2]
			<<  " TolPPM=" << PrecursorsCandidatesArray[idx][5]
			<<  " PIF=" << PrecursorsCandidatesArray[idx][7]
			<< " ChromSim1=" << PrecursorsCandidatesArray[idx][4]
			<< " ChromSim2=" << PrecursorsCandidatesArray[idx][6]
			<<  " Score=" << PrecursorsCandidatesArray[idx][15]<<endl <<endl;;
	}
#endif
	...
	int DeleteStart = MarsLen - PrecursorsCounter;  
	...
	{
		MarsFeature.push_back( MarsFeature[DeleteStart+(int)PrecursorsCandidatesArray[Precursor_i][8]-1]);
		OutputInfo.push_back(OutputInfo[DeleteStart+(int)PrecursorsCandidatesArray[Precursor_i][8]-1]);
	}
...
void MS1Reader::OutputMarsPredictY()
{
	...
	{
		int SameScanNum = 0;
		int currentscan = OutputInfo[i][0];
		vector<int> idx;
		vector<double> Y;
		int newi = 0;
		for(int j=i; j<m_vPredictY.size(); ++j){
			if((int)OutputInfo[j][0] == currentscan){
				Y.push_back(m_vPredictY[j]);
				idx.push_back(SameScanNum);
				++ SameScanNum;
				newi = j;
			}else{
				break;
		...
		sort(idx.begin(),idx.end(), [Y](int k, int l){return Y[k] > Y[l];});
		for(int j=0; j<idx.size(); ++j){
			if (m_vPredictY[i+idx[j]] > MarsThreshold)
			{
				exportInfo.push_back(OutputInfo[i+idx[j]]);	
			}
		}
		i = newi;
	}
...
// 机器学习法对母离子进行分类
void MS1Reader::MacLearnClassify(string modelType)
{
	if (MarsFeature.size() == 0)
	{
		return;
