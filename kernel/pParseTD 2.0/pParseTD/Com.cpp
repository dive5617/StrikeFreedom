...
bool CTools::IsFileExists(string Filename)
{
	FILE *pTest = fopen(Filename.c_str(),"r");
	if (pTest == NULL)
	{
		return false;
	} else {
...
void HEAP::BuildHeap(vector<double> &A, bool isMin){
	int n = A.size();
	// 从叶节点开始调整堆
	for(int i=(n-1)/2; i>=0; --i){
		AjustHeap(A, n, i, isMin);
	}
}
void HEAP::AjustHeap(vector<double> &A, int len, int i, bool isMin){
	int lf = i<<1 + 1;
	int rt = i<<1 + 2;
	int mx = i;
	if(lf < len && A[lf] > A[i]){
	...
	if(mx != i){
		swap(A[i], A[mx]);
		AjustHeap(A, len, mx, isMin);
	}
}
void HEAP::HeapSort(vector<double> &A, bool desc){
	bool isMin = desc;
	BuildHeap(A, isMin);
	for(int i = A.size()-1; i>0; --i){
		swap(A[0], A[i]);
		AjustHeap(A, i, 0, isMin);
	}
}

vector<vector<double>> Tables::s_vf_charge_mz_tol;
void Tables::InitChargeMzTolTable(){
	for(int mz=mz_beg; mz<=mz_end; ++mz){
		vector<double> tmp;
		for(int chg=chg_beg; chg<=chg_end; ++chg){
			double tol = (avgdiv/(chg-1) - avgdiv/chg) / mz;
			tmp.push_back(tol);   //
		}
		s_vf_charge_mz_tol.push_back(tmp);
	}
}